...
def higher_kuramoto_func(_: float, theta: np.ndarray, omega: np.ndarray, K: float, 
    ...
    for e in range(n_edges):
        ...
        j = idx_j[e]
        k = idx_k[e]
        theta_deriv[i] += vals[e] * np.sin(theta[k] + theta[j] - 2 * theta[i])
        
    for I in range(N):
        for J in range(N):
            theta_deriv[I] += 2 * K/N * np.sin(2 * (theta[J] - theta[I]))
                
    # for I in range(N):
    #     for J in range(N):
    #         for L in range(N):
    #             theta_deriv[I] += K/N * np.sin(2 * (theta[L] + theta[J] - 2 * theta[I]))
    #             theta_deriv[I] += 2*K/N * np.sin(theta[L] + theta[J] - 2 * theta[I]) * np.cos(theta[J] - theta[L])
    return theta_deriv
