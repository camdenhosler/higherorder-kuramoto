...
from cuthigher_oscillators import build_sparse_A
from src.perturbations import cuthigher_perturbation, projection_distance
...
def run_trial(seed,N,K,t_start,t_end,ps):
    ...
    fsfpa, ssfpa, dfpa, t = cuthigher_perturbation(node1=node1,node2=node2,t_start=t_start,t_end=t_end,theta_init=theta_init,params=sim_params)

    if t >= t_end:
        return run_trial(seed + 1,N,K,t_start,t_end,ps)
    else:
        return projection_distance(fsfpa,ssfpa,dfpa)
    


def calculate_distribution_parallel(num_trials, N, p):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(run_trial, seed, N, K, t_start, t_end, ps)
            for seed in seeds
        ]
...
def main():
    ...
    N = 20
    p = 0.08
    epis_arr = calculate_distribution_parallel(num_trials,N,p)
