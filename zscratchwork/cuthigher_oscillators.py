...
def higher_kuramoto_func(_: float, theta: np.ndarray, omega: np.ndarray, K: float, 
    ...
    for e in range(n_edges):
        ...
        j = idx_j[e]
        k = idx_k[e]
        theta_deriv[i] += K/N * vals[e] * np.sin(theta[k] + theta[j] - 2 * theta[i]) # + K/N * np.sin(2 * (theta[k] + theta[j] - 2 * theta[i]))
    return theta_deriv
...
def cuthigher_find_fpas(t_start: float,t_end: float, theta_init: np.ndarray, params: Dict[str, Any], func=higher_kuramoto_func) -> Tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        final_derivs = func(t_end, theta_init, params['omega'], params['K'], params['N'],
                             idx_i, idx_j, idx_k, vals)
        return final_phases_mod, final_derivs, t_end
    ...
    return final_phases_mod, final_derivs, solution.t[-1] 
