...
from .oldoscillators import find_fpas
from .allhigher_oscillators import higher_find_fpas

def perturbation(node1: int, node2: int, perturb_strength: np.float64, t_start: float, t_end: float, t_eval:float,theta_init: np.ndarray, params: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:

    cut_find_fpas = partial(find_fpas,t_start=t_start,t_end=t_end,t_eval=t_eval,params=params)

    m_initial_state, _, initial_state, stab1 = cut_find_fpas(theta_init=theta_init)
