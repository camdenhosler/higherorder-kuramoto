...
def kuramoto_func(_: float, theta: np.ndarray, omega: np.ndarray, K: float, 
    ...
    for e in range(n_edges):
        ...
        #theta_deriv[i] += ( K / N ) * vals[e] * np.sin(theta[j] - theta[i])
    return theta_deriv

def zero_deriv(_: float, theta: np.ndarray, omega: np.ndarray, K: float, N: int, idx_i: np.ndarray, idx_j: np.ndarray, vals: np.ndarray):
    theta_deriv = kuramoto_func(_, theta, omega, K, N, idx_i, idx_j, vals)
...
def find_fpas(t_start: float,t_end: float, theta_init: np.ndarray, 
              params: Dict[str, Any], func=kuramoto_func) -> Tuple[np.ndarray, np.ndarray, np.ndarray, bool, bool]:
    """
